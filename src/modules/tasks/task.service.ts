...
import { isPositive } from '../../common/money.ts'
import { db } from '../../db/index.ts'
import { assets, categories, escrows, tasks, users } from '../../db/schema/index.ts'
import type { AuthUser } from '../../plugins/auth.ts'
import { calculateFee, fundEscrow, refundEscrow } from '../../services/escrow.service.ts'
import {
...
const taskIdentity = (idOrPublicId: string) =>
  UUID_RE.test(idOrPublicId) ? eq(tasks.id, idOrPublicId) : eq(tasks.publicId, idOrPublicId)
...
export const createTask = async (owner: AuthUser, input: CreateTaskInput) => {
  if (!isPositive(input.rewardAmount)) throw BadRequest('Reward amount must be greater than zero')
  ...
  if (onchain) {
    ...
    if (!input.deadlineAt) throw BadRequest('On-chain tasks require a deadline')
  ...
  const { fee, total } = onchain
    ? (() => {
        ...
        return { fee: f.toString(), total: (BigInt(input.rewardAmount) + f).toString() }
      })()
      ...
      onchain && cfg
        ? {
            ...
            fundArgs: {
              ...
              deadline: Math.floor(new Date(input.deadlineAt!).getTime() / 1000),
              maxWinners: input.maxWinners ?? 1,
            },
            /** msg.value untuk native, atau nilai approve() untuk ERC-20 */
            deposit: total,
            nextStep: 'Panggil fund() lalu POST /tasks/' + task.publicId + '/publish dengan { txHash }',
          }
        : null,
  }
}
...
export const publishTask = async (
  ...
  if (task.onchainTaskId) {
    assertEvmEnabled()
    if (!opts.txHash) throw BadRequest('txHash of the fund() transaction is required')
    ...
    if (!event)
      throw UnprocessableEntity('Transaction has no BountyFunded event from the escrow contract')
...
export const cancelTask = async (actor: AuthUser, taskId: string) => {
  ...
  if (task.winnersCount > 0)
    throw Conflict('Task already has approved submissions; open a dispute instead')
      ...
      const [updated] = await tx
        ...
        .set({
          ...
          metadata: {
            ...(task.metadata ?? {}),
            refundHint:
              'Panggil refund(' + escrow?.onchainBountyId + ') di kontrak setelah ' +
...
export const listTasks = async (filters: TaskFilters, page: Pagination) => {
  ...
  const where = () =>
    and(
      ...
      filters.minReward ? gte(tasks.rewardAmount, filters.minReward) : undefined,
      filters.q
      ...
      filters.ownerUsername ? eq(users.username, filters.ownerUsername) : undefined,
    )
  ...
  const rows = await db
    .select({
      ...taskListColumns,
      owner: { id: users.id, username: users.username, avatarUrl: users.avatarUrl },
      category: { slug: categories.slug, name: categories.name },
      asset: { symbol: assets.symbol, decimals: assets.decimals, logoUrl: assets.logoUrl },
    })
    .from(tasks)
...
export const getTask = async (
  ...
  const [row] = await db
    .select({
      ...
      asset: { id: assets.id, symbol: assets.symbol, decimals: assets.decimals },
      category: { slug: categories.slug, name: categories.name },
    })
    .from(tasks)
    .innerJoin(users, eq(users.id, tasks.ownerId))
...
export const requireOwnedTask = async (actor: AuthUser, taskId: string) => {
  const [task] = await db.select().from(tasks).where(taskIdentity(taskId)).limit(1)
  if (!task) throw NotFound('Task')
