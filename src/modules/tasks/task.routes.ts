...
const amountSchema = t.String({ pattern: '^[0-9]+$', description: 'Nominal dalam base unit' })
...
export const taskRoutes = new Elysia({ prefix: '/tasks', tags: ['Tasks'] })
  ...
  .get(
    '/',
    ({ query }) => service.listTasks(query, resolvePagination(query)),
    {
      query: t.Composite([
        paginationQuery,
        t.Object({
          q: t.Optional(t.String({ maxLength: 120 })),
          type: t.Optional(t.Union([t.Literal('task'), t.Literal('bounty'), t.Literal('quest')])),
          ...
          sort: t.Optional(
            t.Union([t.Literal('newest'), t.Literal('reward'), t.Literal('deadline')]),
          ),
        }),
      ]),
  ...
  .get('/mine', ({ user, query }) => service.listMyTasks(user, resolvePagination(query)), {
    auth: true,
  ...
  .get(
    '/:id',
    ({ params, currentUser, query }) => service.getTask(params.id, currentUser, query.token),
    {
    ...
    },
    {
      ...
      body: t.Object({
        ...
        visibility: t.Optional(
          t.Union([t.Literal('public'), t.Literal('private'), t.Literal('unlisted')]),
        ),
        ...
        requirements: t.Optional(
          t.Array(t.Object({ label: t.String(), required: t.Boolean() }), { maxItems: 30 }),
        ),
  ...
  .post(
    '/:id/publish',
    ({ user, params, body }) => service.publishTask(user, params.id, body ?? {}),
    {
      auth: true,
      params: t.Object({ id: t.String() }),
      body: t.Optional(
        t.Object({ txHash: t.Optional(t.String({ pattern: '^0x[0-9a-fA-F]{64}$' })) }),
      ),
      detail: {
  ...
  .post('/:id/cancel', ({ user, params }) => service.cancelTask(user, params.id), {
    auth: true,
    params: t.Object({ id: t.String() }),
